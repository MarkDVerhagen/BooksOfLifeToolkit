...
def parse_args():
    ...
    parser.add_argument("--qlora", action="store_true", help="Use QLoRA for fine-tuning instead of full fine-tuning.")
    parser.add_argument("--learning_rate", type=float, default=None, help="Learning rate to use during training.")
...
def main():
    ...
    if local_rank == 0:  # Only initialize wandb in one process
        wandb.init(project=config["project_name"], config=config)
    config = wandb.config

    # Load dataset
    dataset = load_dataset(config["dataset_name"], split=['train', 'test'])
