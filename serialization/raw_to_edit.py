...
def preprocess(domain, data_dir, table_version = "", track_changes_by = ["year"]):
    filetype = "yearly"
    # read in raw data file
    if domain == "education":
        df = pd.read_csv(os.path.join(data_dir, 'raw', f'hoogsteopl_tab{table_version}.csv'), index_col = False)
        output_file_name = "education_bus.csv"
    elif domain == "wealth":
        df = pd.read_csv(os.path.join(data_dir, 'raw', f'veh_tab{table_version}.csv'), index_col = False)
        output_file_name = "wealth_tab.csv"
    elif domain == "employment":
        df = pd.read_csv(os.path.join(data_dir, 'raw', f'spolis_bus{table_version}.csv'), index_col = False)
    ...
    df_cleaned.to_csv(
        os.path.join(data_dir, 'raw', output_file_name), index=False)
...
if __name__ == "__main__":
    ...
    args.add_argument("--domain", type=str, required=True, help="Name of file to transform.")
    args.add_argument("--data_dir", type=str, required=True, help="Directory with data.")
    args.add_argument("--table_version", type=str, required=False, help="Table version (if a subset is used).")
    args.add_argument("--track_changes_by", type=str, required=False, help="Variable to track changes by.")
