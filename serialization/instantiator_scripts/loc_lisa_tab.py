...
from serialization.instantiator_scripts.LisaLocationParagraph import LisaLocationParagraph
...
def get_lisa_loc(rinpersoons: list, conn, table_version: str = '', explicit : bool=True, order: int = 0) -> LisaLocationParagraph: ## 20/8
    """
    This function loads personal attributes for a given rinpersoon (person_id)
    by querying the SQLite database and creating the PersonLocationParagraph object.
    """
    # conn = duckdb.connect(db_name, read_only=True) ## 20/8

    columns_query = f"PRAGMA table_info(loc_lisa_tab{table_version})"
    columns = [row[1] for row in conn.execute(columns_query).fetchall()]

    # Query the database for the person with the given rinpersoon
    query = f"""
    SELECT {', '.join(columns)} FROM loc_lisa_tab{table_version}
    ...
    for rinpersoon in rinpersoons:
        par_dict[rinpersoon] = [LisaLocationParagraph(
            dataset_name="loc_lisa_tab",
